...
ROAToolboxOptions.GridRange = [-2*pi, 2*pi; -2*pi, 2*pi; -2*pi, 2*pi; -4*pi, 4*pi];
...
if iscell(xs)  % element-wise
    ...
    %     m1 = 1; m2 = 1; c1 = 1; c2 = 1; k1 = 0; k2 = 0; L = 1; F = 0; g = 9.81;
    
    % Compute the system derivatives (zero point is at the bottom).
    x1dot = x2;
    x2dot = (-(7*k1)/(7*m1 + 4*m2))*x1 + (-(7*c1)/(7*m1 + 4*m2))*x2 + ((12*k2 + 6*L*g*m2)/(2*L*(7*m1 + 4*m2)))*x3 + ((6*c2)/(L*(7*m1 + 4*m2)))*x4;
    x3dot = x4;
    x4dot = ((6*k1)/(L*(7*m1 + 4*m2)))*x1 + ((6*c1)/(L*(7*m1 + 4*m2)))*x2 + (-(3*(4*k2*m1 + 4*k2*m2 + 2*L*g*m2^2 + 2*L*g*m1*m2))/(L^2*m2*(7*m1 + 4*m2)))*x3 + (-(3*(4*c2*m1 + 4*c2*m2))/(L^2*m2*(7*m1 + 4*m2)))*x4;
