...
if iscell(xs)  % element-wise
    ...
    % Define the angle shift to apply (a non-zero value will move the zero point away from the bottom position. angle_shift = pi sets the vertical position to be zero).
    %     angle_shift = pi;
    angle_shift = 0;
    
    % Set the desired pendulum angle (with respect to the current zero position).
    thetad = 0;
    
    % Make the vertical position of the pendulum zero.
    x1 = x1 + angle_shift;
    thetad = thetad + angle_shift;
    
    % Define the pendulum parameters.
        m = 1; c = 1; k = 0; L = 1; g = 9.81; kmotor = 1; kp = 20;
%     m = 1; c = 1; k = 0; L = 1; g = 9.81; kmotor = 1; kp = 5;
    %     m = 1; c = 1; k = 0; L = 1; g = 9.81; kmotor = 1; kp = 2;
    
    
    % Define the nonlinear simple pendulum dynamics (zero is at the bottom by default).
    x1dot = x2;
    x2dot = (-(k + kmotor*kp)/(m*(L^2)))*x1 + (-c/(m*(L^2)))*x2 + (-g/L)*sin(x1) + (kmotor*kp/(m*(L^2)))*thetad;
