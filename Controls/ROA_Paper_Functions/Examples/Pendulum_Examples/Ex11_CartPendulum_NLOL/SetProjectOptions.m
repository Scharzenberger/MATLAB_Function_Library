...
if iscell(xs)  % element-wise
    ...
    m1 = 1; m2 = 1; c1 = 1; c2 = 1; k1 = 1; k2 = 0; L = 1; F = 0; g = 9.81;

    % Compute the system derivatives.
    x1dot = x2;
    x2dot = (-(7*k1)./(7*m1 - 3*m2*cos(x3).^2 + 7*m2)).*x1 + (-(7*c1)./(7*m1 - 3*m2*cos(x3).^2 + 7*m2)).*x2 + (-(6*k2*cos(x3))./(L*(7*m1 - 3*m2*cos(x3).^2 + 7*m2))).*x3 + (-(7*m2*sin(x3)*L^2 + 12*c2*cos(x3))./(2*L*(7*m1 - 3*m2*cos(x3).^2 + 7*m2))).*x4 + (14*F*L + 6*L*g*m2*cos(x3).*sin(x3))./(2*L*(7*m1 - 3*m2*cos(x3).^2 + 7*m2));
    x3dot = x4;
    x4dot = (-(6*k1*cos(x3))./(L*(7*m1 - 3*m2*cos(x3).^2 + 7*m2))).*x1 + (-(6*c1*cos(x3))./(L*(7*m1 - 3*m2*cos(x3).^2 + 7*m2))).*x2 + (-(3*(4*k2*m1 + 4*k2*m2))./(L^2*m2*(7*m1 - 3*m2*cos(x3).^2 + 7*m2))).*x3 + (-(3*(cos(x3).*sin(x3)*L^2*m2^2 + 4*c2*m2 + 4*c2*m1))./(L^2*m2*(7*m1 - 3*m2*cos(x3).^2 + 7*m2))).*x4 + (3*(2*F*L*m2*cos(x3) + 2*L*g*m2^2*sin(x3) + 2*L*g*m1*m2*sin(x3)))./(L^2*m2*(7*m1 - 3*m2*cos(x3).^2 + 7*m2));
